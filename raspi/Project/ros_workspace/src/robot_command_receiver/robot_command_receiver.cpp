...
int main(int argc, char **argv)
{
  ...
  ros::NodeHandle n;
  chatter_pub= n.advertise<std_msgs::String>("robot_command", 1000);
  ros::Rate loop_rate(100);
  while (ros::ok()){
    std::string command = "{\"id\":\"822453680e284262ba0f73dea1d0a5b8\",\"direction\":\"up\",\"degree\":\"96.69404589067692\",\"radian\":  \"1.6876294678668051\",\"gear\":\"1\",\"status\":\"initialised\",\"securitykey\":\"f50b51935fd14fe0be894a388ed8b5eb\",\"isAutomated\":\"start\"}";
	getCommand(command);
	ros::spinOnce();
  }
  return 0;
...
int main1(int argc, char **argv)
{
  ...
  while(checkInternet()){
	...
	resp = write(sockfd,robot_name_with_id,strlen(robot_name_with_id));
	if (resp < 0) {
	...
	while(1){
	  ...
	  if(length > 0){
		...
		if(getCommand(command)){
		  ack = getAck(1);
		  resp = write(sockfd,ack.c_str(),strlen(ack.c_str()));
		  if (resp < 0){ 
...
int getCommand(std::string command){
  ...
  cout << "isAutomated " << isAutomated << endl;
  if(isAutomated == "start"){
    //runRobotAutonomous();
    std_msgs::String msg;
	std::stringstream ss;
	ss << "start";
    ...
    runMotorForStreeingWheel(command);
    return 1;
  }
  return 0;
}
  ...
  while(remaining.length()>0)	{
	...
	if (colanLocation >0) {
	  ...
	  if(item.substr(1,colanLocation-2) == "isAutomated"){
		isAutomated = item.substr(colanLocation+2,item.length()-colanLocation-3);
	  }
	}
  }
